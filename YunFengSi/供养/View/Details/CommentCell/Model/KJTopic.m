...
- (NSAttributedString *)attributedText{
    ...
    attributedString.font = SystemFontSize(15);
    attributedString.color = [UIColor blackColor];
    attributedString.lineSpacing = KJCommentContentLineSpacing;
...
#pragma mark - Getter
- (NSString *)creatTime{
    NSDateFormatter *fmt = [NSDateFormatter kj_defaultDateFormatter];
#warning CoderMikeHe: 真机调试的时候，必须加上这句
    ...
    // 判断是否为今年
    if (createDate.kj_isThisYear){
        if (createDate.kj_isToday){ // 今天
            NSDateComponents *cmps = [createDate kj_deltaWithNow];
        ...
        } else if (createDate.kj_isYesterday){ // 昨天
