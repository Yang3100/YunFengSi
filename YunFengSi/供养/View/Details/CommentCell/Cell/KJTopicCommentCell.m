...
@implementation KJTopicCommentCell
...
+ (instancetype)cellWithTableView:(UITableView *)tableView{
    ...
    KJTopicCommentCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if (!cell){
//        KJLog(@"....cell嵌套tableView.....创建评论cell...");
...
#pragma mark - 公共方法
- (void)setCommentFrame:(KJCommentFrame *)commentFrame{
    _commentFrame = commentFrame;
    
    KJComment *comment = commentFrame.comment;
...
- (void)_setup{
    ...
    self.contentView.backgroundColor = DefaultBackgroudColor;
}
...
- (void)_setupSubViews{
    ...
    contentLabel.highlightTapAction = ^(UIView *containerView, NSAttributedString *text, NSRange range, CGRect rect){
        // 利用KVC获取UserInfo 其实可以在KJComment模型里面利用 通知告知控制器哪个用户被点击了
        ...
        if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(topicCommentCell:didClickedUser:)]){
            [weakSelf.delegate topicCommentCell:weakSelf didClickedUser:highlight.userInfo[@"KJCommentUserKey"]];
        }
    };
}
