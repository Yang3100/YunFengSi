...
@interface KJTopicCell()< UITableViewDelegate , UITableViewDataSource,KJTopicCommentCellDelegate >
...
@implementation KJTopicCell

+ (instancetype)cellWithTableView:(UITableView *)tableView{
    static NSString *ID = @"TopicCell";
    KJTopicCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if (!cell){
//        KJLog(@"....cell嵌套tableView.....创建话题cell...");
...
#pragma mark - 公共方法
#pragma mark - Setter
- (void)setTopicFrame:(KJTopicFrame *)topicFrame{
    _topicFrame = topicFrame;
    
    KJTopic *topic = topicFrame.topic;
    KJUser *user = topic.user;
    
    // 头像
    self.avatarView.frame = topicFrame.avatarFrame;
    self.avatarView.image = DefaultHeaderImage;
//    [KJWebImageTool setImageWithURL:user.avatarUrl placeholderImage:KJGlobalUserDefaultAvatar imageView:self.avatarView];
...
- (void)_setupSubViews{
    ...
    avatarView.layer.cornerRadius = KJTopicAvatarWH*.5f;
    // 这样写比较消耗性能
    ...
    nicknameLable.text = @"";
    nicknameLable.font = SystemFontSize(14);
    ...
    [thumbBtn setImage:GetImage(@"comment_zan_nor")forState:UIControlStateNormal];
    [thumbBtn setImage:GetImage(@"comment_zan_high")forState:UIControlStateDisabled];
    ...
    thumbBtn.titleLabel.font = SystemFontSize(14);
    ...
    [moreBtn setImage:GetImage(@"comment_more")forState:UIControlStateNormal];
    ...
    createTimeLabel.font = SystemFontSize(14);
    ...
    textContainerInset.top = KJTopicVerticalSpace;
    textContainerInset.bottom = KJTopicVerticalSpace;
    ...
    divider.backgroundColor = DefaultLineColor;
...
- (void)layoutSubviews{
    ...
    // 布局子控件
    self.divider.frame = CGRectMake(10, self.height-KJGlobalBottomLineHeight, self.width-10, KJGlobalBottomLineHeight);
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    KJTopicCommentCell *cell = [KJTopicCommentCell cellWithTableView:tableView];
    KJCommentFrame *commentFrame = self.topicFrame.commentFrames[indexPath.row];
...
#pragma mark - KJTopicCommentCellDelegate
