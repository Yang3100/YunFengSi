...
/// 标识 showPosterGraph
static char KJ_EMPTY_VIEW;

@implementation UIView (KJEmptyView)

#pragma mark - Getter & Setter
- (void)setEmptyDataView:(KJEmptyDataView *)emptyDataView{
    [self willChangeValueForKey:@"emptyDataView"];
    
    //关联对象
    objc_setAssociatedObject(self, &KJ_EMPTY_VIEW,
                             emptyDataView,
...
- (KJEmptyDataView *)emptyDataView{
     return objc_getAssociatedObject(self, &KJ_EMPTY_VIEW);
}
...
- (void)kj_configEmptyViewWithType:(KJEmptyDataViewType)type emptyInfo:(NSString *)emptyInfo errorInfo:(NSString *)errorInfo offsetTop:(CGFloat)offsetTop hasData:(BOOL)hasData hasError:(BOOL)hasError reloadBlock:(void(^)(void))reloadBlock{
    ...
    if (self.emptyDataView == nil) {
        ...
        self.emptyDataView = [[KJEmptyDataView alloc] initWithFrame:CGRectMake(0.0, 0.0, CGRectGetWidth(superviewBounds), CGRectGetHeight(superviewBounds))];
    }
