...
- (void)configEmptyViewWithType:(KJEmptyDataViewType)type emptyInfo:(NSString *)emptyInfo errorInfo:(NSString *)errorInfo offsetTop:(CGFloat)offsetTop hasData:(BOOL)hasData hasError:(BOOL)hasError reloadBlock:(void(^)(void))reloadBlock{
    ...
    self.reloadButton.hidden = KJObjectIsNil(reloadBlock);
    self.reloadBlock = reloadBlock;
    /// 没有数据的情况 1. 确实没有数据  2. 请求出错导致无数据
    self.hidden = NO;
    
    NSInteger random = [NSObject kj_randomNumber:0 to:1];
    UIImage *image = nil;
...
- (void)_setupSubViews{
    ...
    [reloadButton setTitleColor:MainColor forState:UIControlStateNormal];
    [reloadButton setTitle:@"重新连接" forState:UIControlStateNormal];
    reloadButton.layer.cornerRadius = 4.f;
    reloadButton.layer.borderColor = DefaultLineColor.CGColor;
