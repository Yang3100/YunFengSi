...
- (void)tableViewDidTriggerHeaderRefresh{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        /**
         ///
         [KJHttpRequestTool POST:UFMapListUrl params:params completion:^(NSURLSessionDataTask *task, id responseObject) {
         /// 请求成功的回调
        ...
        KJEmptyDataViewType emptyType = [NSObject kj_randomNumber:0 to:8];
        if (requestState < 0) {
            ...
            [self.tableView kj_configEmptyViewWithType:emptyType emptyInfo:nil errorInfo:nil offsetTop:250 hasData:self.dataSource.count>0 hasError:hasError reloadBlock:NULL];
        }else{
            /// 请求失败的回调，
            /// 客户端一般只需要关心出错的原因是：
            /// - 网络问题
            ...
            @weakify(self);
            [self.tableView kj_configEmptyViewWithType:emptyType emptyInfo:nil errorInfo:nil offsetTop:250 hasData:self.dataSource.count>0 hasError:YES reloadBlock:^{
                @strongify(self);
                [MBProgressHUD kj_showProgressHUD:@"模拟网络加载ing.." addedToView:self.view];
                [self tableViewDidTriggerHeaderRefresh];
            }];
